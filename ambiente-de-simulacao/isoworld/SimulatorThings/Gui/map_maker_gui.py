...
class MapMakerGui(Gui):
    def __init__(self, screen, images) -> None:
        ...
        self.painel = UIPanel(
            relative_rect=pygame.Rect((0, 0), (200, 640)), manager=self.manager
        )
    ...
    def __init_name_field(self):
        ...
        elements.append(
            UITextEntryLine(
                ...
                object_id=ObjectID("#map_name_text_field"),
            )
        )
    ...
    def __init_tileset(self):
        ...
        for i, tile in enumerate(self.tiles_images):
            ...
            tileset.append(
                TilesetOption(self.manager, temp_x, temp_y, tile, self.painel, i)
            )
    ...
    def __init_buttons(self):
        ...
        buttons.append(
            UIButton(
                relative_rect=pygame.Rect((30, 590), (70, 50)),
                text="Apagar",
                container=self.painel,
                object_id=ObjectID("#mouse_changer_R"),
                manager=self.manager,
            )
        )
        buttons.append(
            UIButton(
                relative_rect=pygame.Rect((105, 590), (80, 50)),
                text="Salvar",
                container=self.painel,
...
class TilesetOption:
    def __init__(self, manager, x, y, image, container, id) -> None:
        ...
        self.uibutton = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect((x - 3, y - 3), (60, 70)),
            text="",
            container=container,
            object_id=ObjectID(f"#mouse_changer_{id}"),
            manager=manager,
        )
        self.ui_tile_image = UIImage(
            relative_rect=pygame.Rect((x, y), (55, 64)),
            image_surface=image,
            container=container,
        )
